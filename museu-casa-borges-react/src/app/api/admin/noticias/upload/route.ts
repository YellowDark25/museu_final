...
import { uploadNoticiaImageToSupabase } from "@/lib/storage/upload-noticias-supabase"
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const baseName = path.basename(file.name, extension).trim() || "noticia"

    const url = await uploadNoticiaImageToSupabase({
      extension,
      buffer,
