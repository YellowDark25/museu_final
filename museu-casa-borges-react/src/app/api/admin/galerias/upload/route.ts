...
import { uploadGaleriaImageToSupabase } from "@/lib/storage/upload-galerias-supabase"
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const subfolder = (formData.get("subfolder") as string | null) ?? "itens"
    ...
    const baseName = path.basename(file.name, extension).trim() || "foto"

    const url = await uploadGaleriaImageToSupabase({
      extension,
      buffer,
      subfolder,
